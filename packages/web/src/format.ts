/**
 * Euro FORMATTING lives in `@finance/shared` as `formatEur` (spec 004), so the
 * API and the UI cannot disagree about it. What stays here is the input side —
 * parsing what a human types — which only the UI needs.
 */

/**
 * The inverse of `formatEur`: a typed euro amount to integer cents.
 *
